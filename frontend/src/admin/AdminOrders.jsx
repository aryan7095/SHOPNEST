...
// Admin page for viewing all orders and updating their fulfillment status
const AdminOrders = () => {
  const { user } = useContext(AuthContext);
  const [orders, setOrders] = useState([]);

  // Fetch all orders on mount (or whenever the user changes)
  useEffect(() => {
  ...
  // Updates an order's status via the API, then updates local state to reflect the change
  // without needing to refetch the entire orders list
  const updateStatus = async (id, status) => {
    ...
    if (res.ok) {
      // Optimistically update just the changed order in local state
          ...
          <tbody>
            {orders.map(order => (
              <tr key={order._id} style={rowStyle}>
                {/* Show a shortened order ID for readability */}
                <td style={tdStyle}>{order._id.substring(0, 8)}...</td>
                {/* Fallback text if the user who placed the order was deleted */}
                ...
                <td style={tdStyle}>
                  {/* Status dropdown — changing it immediately triggers an API update */}
                  <select 
                    value={order.status} 
